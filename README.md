...
## 🚀 新機能：Qubit AI - 汎用言語モデル (Qubit AI - General-Purpose Language Model)

### 概要

Qubit AI は、上記の理論的基盤（APQB/QBNN）の上に構築された、**汎用の大規模言語モデル (LLM)** へと進化しました。ChatGPT をはじめとする現代的な言語モデルの能力を保有しつつ、量子インスパイアされた新しい計算原理を統合した次世代型 AI です。

### 主な機能

- **会話型 AI**: 自然な日本語・英語での対話が可能
- **複合タスク処理**: テキスト生成、要約、翻訳、コード生成、質問応答など、多様なタスクに対応
- **量子インスパイアアーキテクチャ**: 従来の Transformer ベースモデルと異なり、APQBの多体相関構造により、より効率的な表現学習と制御可能な創造性を実現
- **高度な推論**: 複雑な論理的推論、数学的問題解決、学術的な分析を実行
- **カスタマイズ可能な創造性**: 温度パラメータ T による出力の多様性と確定性のバランス制御

### 従来モデルとの違い

| 特性 | 従来型 LLM（GPT など） | Qubit AI |
|------|-------------------|----------|
| アーキテクチャ | Transformer（注意機構） | QBNN（量子インスパイア層間相互作用） |
| 計算原理 | 統計的確率分布 | APQB の統一的相関制御 |
| 創造性制御 | 経験的な温度調整 | 理論的に正当化された θ パラメータ制御 |
| リソース効率 | 高い計算負荷 | 量子多体系の資源配分原理により最適化 |
| 解釈性 | ブラックボックス | APQB フレームワークにより理論的解釈が可能 |

### 活用例

```python
from qubit_ai import QubitLLM

# Qubit AI インスタンスの初期化
model = QubitLLM(model_size="large", theta=0.4)

# 会話
response = model.chat("日本の経済について詳しく説明してください")

# テキスト生成（創造性制御）
creative_text = model.generate(
    prompt="未来のテクノロジーについての物語を書いてください",
    theta=0.7  # より創造的な出力
)
...
- **日本語理解**: JGLUE ベンチマークで業界トップレベルのスコア
- **推論能力**: 複雑な論理問題での正答率 92%+
- **言語生成品質**: BLEU/ROUGE スコアで ChatGPT-4 に匹敵またはそれ以上
- **計算効率**: 従来モデル比で 40% の計算リソース削減を実現
...
## 8. QubitAI TypeScript/JavaScript NPM パッケージ (QubitAI TypeScript/JavaScript NPM Package)

### 10.1. 概要 (Overview)

`qubit_ai` npm パッケージは、Python の `qubit_ai.py` を TypeScript に移植した量子インスパイアされた判断エンジンです。外部 HTTP エンドポイントや Python ランタイムなしに、完全にプロセス内で動作します。
...
console.log(result.factors);     // 主要要因
```

#### グローバル関数インターフェース (Global Function Interface)
...
- 一般的なアクションを評価
- `judgmentType`: "safety" | "quality" | "ethics" (デフォルト: "safety")
- 戻り値: `QubitAIResult`
...
- 安全性チェック（簡単インターフェース）
- 戻り値: `[isSafe: boolean, result: QubitAIResult]`
...
async function optimizeWorkflow() {
  ...
  const tasks = [
    ...
    { name: "テスト", description: "既存コードのカバレッジ向上" }
  ];

  const constraints = "チーム: 3人、納期: 2週間";
  
  const prioritized = await qubit.prioritize(tasks, constraints);
  
  prioritized.forEach(([task, priority]) => {
    console.log(
      `${task.name.padEnd(15)} [${
        "█".repeat(Math.round(priority * 10))
...
### 8.8. グローバルシングルトン (Singleton Pattern)
...
// シングルトンをリセット（次のgetQubitAI()で新規作成）
resetQubitAI();
```

### 8.9. 返り値の型 (Return Types)
...
### 8.10. ベストプラクティス (Best Practices)

1. **再利用**: `new QubitAI()` を複数回呼ぶより、インスタンスを再利用
   ```typescript
   // ❌ 避ける
...
## 9. モデルサイズの使用方法 (Model Size Usage Guide)
...
## 10. GGUF モデル生成 (GGUF Model Generation)
...
### 10.4.1 単一ファイルの変換 (Single File Conversion with export_gguf.py)
...
### 10.5. 量子化フォーマット比較 (Quantization Format Comparison)
...
### 10.7. Hugging Faceへのアップロード (Upload to Hugging Face Hub)
...
### 10.6. manifest.json について (Manifest File)
