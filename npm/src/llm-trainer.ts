  ...
  async loadDataset(opts: {
    ...
    try {
      for await (const example of this.datasetLoader.streamRows({
        ...
        const promptField = opts.promptField ?? this.detectPromptField(example);
        const completionField = opts.completionField ?? this.detectCompletionField(example);

        if (promptField && completionField) {
          examples.push({
            prompt: String(example[promptField] ?? ""),
            completion: String(example[completionField] ?? ""),
          });
        }
