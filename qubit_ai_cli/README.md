...
# バックエンドを指定
python -m qubit_ai_cli.app --backend neuroquantum         # 既定 (会話エンジン, pure Python)
python -m qubit_ai_cli.app --backend neuroquantum-model   # 学習済みモデル (torch + checkpoint)
python -m qubit_ai_cli.app --backend quantum              # 判断エンジン (pure Python)
python -m qubit_ai_cli.app --backend echo                 # 動作確認用
```
