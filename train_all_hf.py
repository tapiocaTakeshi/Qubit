#!/usr/bin/env python3
"""全HuggingFaceデータセットの全サンプルで学習するスクリプト。
小〜中規模データセットは全件、巨大データセットは大量サンプルを使用。
"""
...
DATASETS = [
    # 全件使用（中規模）
    {"id": "fujiki/japanese_alpaca_data", "col": "output", "max_samples": None},
    {"id": "FreedomIntelligence/alpaca-gpt4-japanese", "col": "conversations", "max_samples": None},
    {"id": "kunishou/oasst1-chat-44k-ja", "col": "conversations", "max_samples": None},
    {"id": "shi3z/Japanese_wikipedia_conversation_100K", "col": "conversations", "max_samples": None},
    # 巨大データセット（大量サンプル）
    {"id": "izumi-lab/llm-japanese-dataset", "col": "output", "max_samples": 100000},
]

CC100_MAX = 50000


def extract_texts(ds, text_column, max_samples=None):
    """データセットからテキスト抽出。"""
    texts = []
...
def main():
    ...
    # === Phase 1: 全データセットからテキスト取得 ===
    print(f"\n{'='*60}")
    print(f"Phase 1: 全HuggingFaceデータセットからデータ取得")
    print(f"{'='*60}")
    ...
    for ds_info in DATASETS:
        ds_id = ds_info["id"]
        max_s = ds_info["max_samples"]
        label = f"全件" if max_s is None else f"最大{max_s}"
        print(f"\n  Loading {ds_id} ({label})...")
        try:
            ds = load_dataset(ds_id, split="train")
            texts = extract_texts(ds, ds_info["col"], max_s)
    ...
    # CC100-ja (streaming)
    print(f"\n  Loading range3/cc100-ja (streaming, max {CC100_MAX})...")
    try:
    ...
    # === Phase 3: 学習 ===
    print(f"\n{'='*60}")
    print(f"Phase 3: 学習 ({EPOCHS} epochs, batch={BATCH_SIZE}, accum={GRAD_ACCUM})")
    ...
    new_checkpoint = {
        ...
        "config": cfg,
        "training_log": prev_log + [{"type": "hf-all-datasets", "info": f"loss={best_loss:.4f}", "datasets": loaded_datasets}],
