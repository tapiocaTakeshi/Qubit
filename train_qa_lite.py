#!/usr/bin/env python3
"""
QA軽量学習: データ数を絞り、CPU環境で実用的な時間内に完了する。
手作りQAを重視し、外部データセットからはサンプリング。
"""
import os
import sys
import torch
...
# CPU-optimized: small data, more epochs
EPOCHS = 15
LR = 4e-5
BATCH_SIZE = 4
GRAD_ACCUM_STEPS = 4
WARMUP_STEPS = 20
GRAD_CLIP = 1.0
SAMPLE_PER_DATASET = 3000  # sample from each external dataset
...
def main():
    ...
    print(f"Config: embed={config['embed_dim']}, layers={config['num_layers']}")
    ...
    # Load QA data (sampled)
    print("\n=== Loading QA datasets (sampled) ===")
    qa_texts = []

    datasets_info = [
        ("fujiki/japanese_alpaca_data", "alpaca"),
        ("FreedomIntelligence/alpaca-gpt4-japanese", "conv"),
        ("kunishou/oasst1-chat-44k-ja", "conv"),
    ]

    for ds_id, fmt in datasets_info:
        ...
        try:
            ...
            sampled = all_items[:SAMPLE_PER_DATASET]
            qa_texts.extend(sampled)
            print(f"    -> {len(sampled)}/{len(all_items)} sampled")
        except Exception as e:
            print(f"    -> ERROR: {e}")

    # Hand-crafted QA (重点強化)
    crafted = [
        "質問: 日本の首都はどこですか？\n回答: 日本の首都は東京です。",
        ...
        "質問: 地球の年齢はどれくらい？\n回答: 地球の年齢は約46億年です。",
        "質問: 地球は何歳ですか？\n回答: 地球は約46億歳です。放射性同位元素の測定により推定されています。",
        "質問: DNAとは何ですか？\n回答: DNAはデオキシリボ核酸の略で、生物の遺伝情報を保持する分子です。二重らせん構造を持ちます。",
        "質問: DNAとは？\n回答: DNAは生物の遺伝情報を担う分子で、二重らせん構造を持っています。アデニン、チミン、グアニン、シトシンの4つの塩基で構成されます。",
        ...
        "質問: 織田信長はどんな人物ですか？\n回答: 織田信長は戦国時代の武将で、天下統一を目指しました。1582年に本能寺の変で明智光秀に討たれました。",
        "質問: 人間の体で一番大きい臓器は？\n回答: 人間の体で一番大きい臓器は皮膚です。体全体を覆い、外部から身体を保護しています。",
        "質問: 人間の体で最も大きい臓器は何ですか？\n回答: 皮膚が人間の体で最も大きい臓器です。面積は約1.6〜1.8平方メートルあります。",
        "質問: 三角形の内角の和は？\n回答: 三角形の内角の和は180度です。",
        "質問: 北海道の特徴を教えてください。\n回答: 北海道は日本最北の島で、面積は日本最大です。農業、酪農、漁業が盛んで、観光地も多いです。",
        "質問: 水は何度で沸騰しますか？\n回答: 水は標準気圧のもとで100度（摂氏）で沸騰します。",
        "質問: ニューラルネットワークとは？\n回答: ニューラルネットワークは人間の脳の神経回路を模倣した機械学習モデルです。深層学習の基盤技術です。",
    ]
    # Repeat crafted QA heavily
    for _ in range(80):
        qa_texts.extend(crafted)
    print(f"  + {len(crafted) * 80} crafted QA samples")
