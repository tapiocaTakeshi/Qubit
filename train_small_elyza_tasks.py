#!/usr/bin/env python3
"""
NeuroQuantum "small" モデルを日本語指示データセット ELYZA-tasks-100
(elyza/ELYZA-tasks-100) で学習し、チェックポイントを Hugging Face Hub に
アップロードする。

ELYZA-tasks-100 は ELYZA 社が公開した日本語の指示追従評価用データセットで、
`input`(指示) / `output`(模範解答) / `eval_aspect`(評価観点) の 3 列を持つ
100 件の `test` split から構成される。本スクリプトは各サンプルを

    <USER> {input}
    <ASSISTANT> {output}

という会話形式へ整形して言語モデルの学習に用いる。100 件と小規模なため、
OASST 等で事前学習済みのチェックポイントから `--resume` で追加学習(指示
チューニング)する用途を想定している。

使い方:
    # 1. 既存チェックポイントから ELYZA-tasks で追加学習 (推奨):
    python train_small_elyza_tasks.py --resume \
        --ckpt-name neuroq_small_oasst_ja_checkpoint.pt \
        --tokenizer-prefix neuroq_small_oasst_ja_tokenizer \
        --epochs 5

    # 2. ELYZA-tasks のみで新規学習 (動作確認用):
    python train_small_elyza_tasks.py --epochs 3 --vocab-size 4000

    # 3. 学習 + HF アップロード:
    HF_TOKEN=hf_xxx python train_small_elyza_tasks.py --resume \
        --epochs 5 --upload --repo-id tapiocatakeshi/Qubit

備考:
    - ELYZA-tasks-100 の split 名は `test` のため、デフォルトで `test` を読み込む。
    - `elyza/ELYZA-tasks-100-TV` など列構成が同じ派生データセットも `--dataset-id`
      / `--split` で指定可能。
"""
import argparse
import json
...
DEFAULT_SPLIT = "test"  # ELYZA-tasks-100 は test split のみを公開している
DEFAULT_REPO_ID = "tapiocatakeshi/Qubit"
DEFAULT_CHECKPOINT_NAME = "neuroq_small_elyza_tasks_checkpoint.pt"

progress = ProgressLogger("train_small_elyza_tasks")


def extract_elyza_texts(ds, max_samples: int):
    """ELYZA-tasks-100 の `input` / `output` 列を会話テキストへ整形する。

    各サンプルを `<USER> 指示\n<ASSISTANT> 模範解答` の 1 ターン会話として返す。
...
def upload_checkpoint_to_hf(ckpt_path: str, repo_id: str, hf_token: str, tokenizer_path: str | None = None):
    ...
    api.upload_file(
        ...
        repo_id=repo_id,
        commit_message=f"Add {ckpt_basename} (NeuroQuantum small / ELYZA-tasks-100)",
    )
...
def parse_args():
    ...
    p.add_argument("--dataset-id", default=DATASET_ID, help="HF データセット ID (デフォルト: ELYZA-tasks-100)")
    p.add_argument("--split", default=DEFAULT_SPLIT, help="読み込む split (ELYZA-tasks-100 は test)")
    p.add_argument(
        "--max-samples",
        type=int,
        default=0,
        help="使用する最大サンプル数。0 または負値で全件 (約 100) を使用",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="N バッチごとに中間チェックポイントを保存 (0 で無効)。ELYZA は小規模なので既定は無効",
    )
    p.add_argument(
        "--resume",
        action="store_true",
        help="既存チェックポイント (--ckpt-name) と保存済みトークナイザーから学習を再開/追加学習する。"
        " 事前学習済みモデルへ ELYZA-tasks で指示チューニングする場合に使用",
    )
    p.add_argument(
        "--reset-epochs",
        action="store_true",
        help="--resume 時に training_log のエポック数を無視し、重みのみ流用して 0 エポック目から学習する。"
        " 別データセットで事前学習したチェックポイントを ELYZA で追加学習する際に推奨",
    )
    ...
    p.add_argument("--tokenizer-prefix", default="neuroq_small_elyza_tasks_tokenizer")
    return p.parse_args()
...
def main():
    ...
    texts = extract_elyza_texts(ds, args.max_samples)
    progress.log_dataset_loaded(args.dataset_id, len(texts))
    if not texts:
        raise RuntimeError("ELYZA-tasks からテキストを取得できませんでした")
    ...
    if resume_ckpt is not None:
        ...
        if args.reset_epochs:
            # 別データセットで事前学習したチェックポイントを ELYZA で追加学習する場合、
            # エポックカウントはリセットしつつ過去ログは履歴として保持する。
            start_epoch = 0
            progress.info("--reset-epochs: 重みのみ流用し、ELYZA 学習を 0 エポック目から開始します。")
        else:
    ...
    history_path = os.path.join(
        ...
        "training_history_small_elyza_tasks.json",
    )
    history = {
        "architecture": "neuroquantum",
        "model_size": "small",
